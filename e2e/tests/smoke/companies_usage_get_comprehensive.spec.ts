 ...
 * Note: This appears to be a duplicate of companies_usage_comprehensive.spec.ts but kept for completeness
 */

const API_BASE_URL = "http://127.0.0.1:5050";

test.describe("GET /companies/{companyId}/usage (Alternative) - Comprehensive Tests", () => {
	...
	test.describe("200 Success Responses", () => {
		...
		test("should fetch usage with date filters", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/companies/${testCompanyId}/usage?day=1&month=1&year=2026`,
				{
					headers: {
						Authorization: `Bearer ${validAccessToken}`,
					},
