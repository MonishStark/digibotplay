...
class Chat {
	...
	async getChatHistoriesForUserBySpecificScope(
		userId,
		teamId,
		resourceId,
		scope,
	...
	async addAIReplyToUserQueries(userQueries, aiQueries) {
		...
		for (const userQuery of userQueries) {
			...
			let replyData = await aiQueries.find(
				(queryData) => queryData.parent == userQuery.id,
			);
			if (replyData) {
				...
				.then(async (chatMessagesList) => {
					this.getNoOfPastMessageToBeAdded()
						.then(async (noOfPastMessageToAdd) => {
							...
							const userQueries =
								await this.extractUserQueries(filteredConversation);
							const aiAnswers =
								await this.extractAIAnswers(filteredConversation);
							const chatHistories = await this.addAIReplyToUserQueries(
								userQueries,
								aiAnswers,
							);
	...
	getChatMessages(chatId, before, limit = 50) {
		return new Promise((resolve, reject) => {
			...
			query.then((messages) => resolve(messages.reverse())).catch(reject);
		});
	}
					...
					this.getNoOfPastMessageToBeAdded()
						.then((noOfPastMessageToAdd) => {
							...
							const startIndex = Math.max(
								0,
								listLastIndex - noOfPastMessageToAdd,
							);
