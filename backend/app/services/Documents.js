...
class Documents {
	...
	createDefaultFoldersForTeam(teamAlias, defaultFoldersArray) {
		defaultFoldersArray.forEach((folder) => {
			const folderPath =
				`${process.env.DOCUMENT_PATH}/` + teamAlias + "/" + folder;
	...
	async getChildFoldersAndFiles2(
		parentId,
		teamId,
		search = "",
		offset = 0,
		...
		try {
			...
			const baseQuery = this.dbConnection("documents").where({
				parentId,
				teamId,
				isTrashed: false,
			});
						...
						return this.dbConnection("users")
							...
							.then((owner) => {
								...
								return this.dbConnection("users_meta")
									...
									.first()
									.then((userImage) => {
										foldersAndFiles[index]["avatarUrl"] =
											`${process.env.USER_PROFILE_IMAGE_URL}/${userImage?.metaValue || "default_avatar.png"}`;
	...
	createFolder(folderName, tooltip, isDefault, parentId, teamId, userId) {
		return new Promise((resolve, reject) => {
	...
	createFile(fileName, parentId, teamId, source) {
		return new Promise((resolve, reject) => {
			const dateTime = new Date();
			this.dbConnection("documents")
				.insert({
			...
			try {
				...
				while (true) {
					...
					for (const content of contents) {
						let tempData = await this.getChildFoldersAndFiles(
							content.id,
							content.teamId,
						);
		...
		const backgroundDelete = async () => {
			try {
				...
				while (streamingRows > 0) {
					...
					if (
						metadata.streamingBuffer &&
						metadata.streamingBuffer.estimatedRows > 0
					) {
						streamingRows = parseInt(
							metadata.streamingBuffer.estimatedRows,
							10,
						);
				...
				.then(async (uuid) => {
					if (filesList.length > 0) {
						const folderPath = path.resolve(
							`${process.env.DOCUMENT_PATH}/${uuid}`,
						);
				...
				.then(async (uuid) => {
					this.getFileData(fileId)
						.then(async (file) => {
							...
							logger.info(
								`FileName: ${file.name} and ID: ${fileName} exists on cloud : ${exists}`,
							);
							if (exists) {
								const res = await storage
									.bucket(process.env.GOOGLE_STORAGE_BUCKET_NAME)
		...
		return new Promise(async (resolve, reject) => {
			try {
				...
				if (isNaN(day) || isNaN(month) || isNaN(year)) {
					return reject(
						new Error("Day, month, and year must be valid numbers"),
					);
				}
				...
				const companyUsers = await this.dbConnection(
					"user_company_role_relationship",
				).where({ company: companyId });
				const userIds = companyUsers.map((user) => user.userId);

				// Count queries for the company users
				const chatHistories = await knex("chat_histories")
					.select("id")
				...
				const teamsCount = await knex("teams")
					.where({ companyId })
					.whereBetween("created", [startOfDay, endOfDay]);
				const noOfUsers = {
	...
	getStatisticsDetailForCompanyForMonth(companyId, month, year) {
		return new Promise(async (resolve, reject) => {
			try {
				const savedData = await knex("usage_statistics")
					.select("*")
					.where({ statId: companyId, month, year, type: "company" });
				if (savedData[0]) {
					...
					const totalStorage = { used: storageUsed, limit: storageLimit };
					const limit = Number(
						await getAdminSetting("RECORDING_MONTHLY_LIMIT"),
					);
					const recordingCountData = await knex("recordings")
						.select("*")
						.whereBetween("created", [startOfMonth, endOfMonth])
					...
					const usersLimit = await getAdminSetting("MAX_USERS");
					const noOfUsers = {
						current: usersData.length || 0,
						limit: Number(usersLimit),
					};
	...
	getStatisticsDetailForUserForMonth(userId, month, year) {
		return new Promise(async (resolve, reject) => {
			try {
				const savedData = await knex("usage_statistics")
					.select("*")
					.where({ statId: userId, month, year });
				if (savedData[0]) {
					const parsedData = JSON.parse(savedData[0].data);
					const result = {
						queries: parsedData.queries,
			...
			team
				...
				.then(async (oldAlias) => {
					if (oldAlias != newAlias) {
						await fs2.rename(
							path.join(docBasePath, oldAlias),
							path.join(docBasePath, newAlias),
						);
						resolve(1);
					} else {
						resolve(1);
		...
		return new Promise(async (resolve, reject) => {
			try {
				...
				const outputFilename = path.resolve(
					`${process.env.TMP_CSV_PATH}/${fileName}.csv`,
				);
		...
		return new Promise(async (resolve, reject) => {
			try {
				const htmlFilePath = path.join(
					path.resolve(process.env.DOCUMENT_PATH),
					uuid,
				);
				await fs2.writeFile(
					path.join(htmlFilePath, `${fileName}.html`),
					htmlString,
				);
	...
	extractTextFromHtmlStringAndCreateTextFile(htmlString, userId, fileName) {
		return new Promise(async (resolve, reject) => {
			try {
				const tmpTextFileBasePath = path.join(
					path.resolve(process.env.TMP_TXT_PATH),
					`${userId}`,
				);
				const options = {
					wordwrap: false,
				...
				await fs2.writeFile(
					path.join(tmpTextFileBasePath, `${fileName}.txt`),
					text,
				);
	...
	async createAndStoreEmbeddingsOnIndex(
		documents,
		namespace,
		fileId,
		fileExtension,
			...
			async function processEmbeddingBatch(
				...
				try {
					...
					const response = await genAI.models.embedContent({
						...
						contents: batch.map((batchItem) => ({
							parts: [{ text: batchItem.content }],
						})),
					});
	...
	async queryIndex(uuid, parentId, chatId, question) {
		...
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				res = await customQuerying.queryIndexByCustomQuerying(
					question,
					uuid,
					chatId,
				);
				...
				if (!(await this.isOutOfContextAnswer(res.result))) {
					fileList = await this.extractMetadataFromDocuments(
						res.sourceDocuments,
					);
				}
				...
				if (delimitedText) {
					...
					const BIGQUERY_FAILURE_RESPONSE = await getAdminSetting(
						"BIGQUERY_FAILURE_RESPONSE",
					);
					console.log(
						"Adding failure response as no valid answer generated",
						chatId,
					);
					chat
						.addMessagesToTheChatHistory(
