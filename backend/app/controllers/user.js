...
class UsersController {
	static async getStoredRefreshToken(userId) {
		const row = await knex("user_tokens").where({ userId: userId }).first();
		...
		try {
			...
			jwt.verify(
				...
				async (err, decoded) => {
					...
					const storedToken =
						await UsersController.getStoredRefreshToken(userId);
	...
	static async verifySocialSignupPKCE({ request, response }) {
				...
				if (request.body.phoneNumber) {
					...
					if (!phoneRegex.test(request.body.phoneNumber)) {
						errors.push({
							field: "phoneNumber",
							issue:
								"Phone number must contain only numbers and be 6–15 digits long",
						});
					}
				}
	...
	static async checkIfEmailExist(request, response) {
		...
		// Check if email exists and is not empty/null
			...
			user
				...
				.then((res) => {
					if (res.stat == "valid") {
										...
										user
											...
											.then((type) => {
												user.getCompanyRole(userData.id).then((roleData) => {
													user
														.getCompanyDetails(roleData.company)
														.then(async (companyData) => {
															...
															const refreshToken =
																UsersController.createRefreshToken(
																	refreshPayload,
																);
															...
															if (type === "solo") {
																...
																userData = {
																	...
																	role: roleData.role,
																	accountType: type,
																	companyLanguage: companyData.language,
																};
															} else {
																userData = {
																	...userData,
															...
															logger.info(
																`Authentication success for ${request.body.email}`,
															);
															logger.debug(
																JSON.stringify({
																	success: true,
											...
											.catch((err) => {
												logger.warn(
													`Authentication failed for ${request.body.email}`,
												);
												logger.error(err);
												logger.debug(
													JSON.stringify({
														success: false,
														message: request.t("loginFailed"),
													}),
												);
												return response.status(200).send({
													success: false,
												...
												user.getMailTemplate(9).then(async (data) => {
													...
													transporter.sendMail(
														...
														function (error, info) {
															if (error) {
																logger.warn(
																	`Failed to send OTP for ${request.body.email}`,
																);
																logger.error(error.message);
																return console.log(error);
															}
															logger.info(
																`OTP sent successfully for ${request.body.email}`,
															);
														},
													);
												});
						...
						logger.warn(
							`Authentication failed, account locked for ${request.body.email}`,
						);
						logger.debug(
							JSON.stringify({
								success: false,
	...
	static async validateGoogleOTPAndAuthenticateUser(request, response) {
		...
		if (request.body.email && request.body.otp) {
			...
			user
				.validateGoogleCredentialAndOtp(request.body.email, request.body.otp)
				.then((res) => {
					if (res == "valid") {
						logger.info(`Valid credentials provided by ${request.body.email}`);
						user.getUserDetails(request.body.email).then((data) => {
			...
			if (loginType === "social") {
				...
				if (!code_verifier) {
					return res.status(400).json({
						...
						message: "Invalid or missing fields",
						details: [
							{ field: "code_verifier", issue: "This field is required" },
						],
					});
				}
	...
	static async validateOTPAndAuthenticateUser(req, res) {
		...
		if (!email || !otp) {
			const details = [];
			if (!email)
				details.push({ field: "email", issue: "This field is required" });
		...
		try {
			...
			if (otpStatus !== "valid") {
				...
				if (otpStatus === "locked")
					return sendError(401, req.t("accountLocked"));
		...
		if (request.body.userId) {
			try {
				...
				if (integration?.login == 1) {
					...
					const siteURL =
						siteResponse.data.sites[0]?.URL?.replace("https://", "") || "";
				...
				return response
					.status(200)
					.json({ success: false, login: false, error: error.message });
			}
		} else {
			return response.status(400).send({
				success: false,
			...
			try {
				...
				if (integration?.login == 1) {
					...
					if (slackResponse.data.ok) {
						...
						logger.error(
							"Failed to retrieve Slack files:",
	...
	static async removeUserPermanently(request, response) {
		...
		if (request.params.userId) {
			...
			user
				.removeUser(request.params.userId)
				.then((res) => {
					if (res == 1) {
												...
												.then((data) => {
													...
													user
														...
														.then(async (roleData) => {
															...
															logger.info(
																`Account created for ${request.body.email}`,
															);
															logger.debug(
																JSON.stringify({
																	success: true,
